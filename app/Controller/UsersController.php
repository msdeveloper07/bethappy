...
class UsersController extends AppController {
    ...
    function admin_login() {
        ...
        try {
            ...
            //$this->log('Admin Login Session Check');
            //$this->log($this->Session->check('Auth'));
...
//            }
    ...
    function affiliate_login() {
        ...
        if (!empty($this->request->data)) {
            $user = $this->User->find('first', array('conditions' => array('username' => $this->request->data['User']['username']), 'recursive' => 1));
        ...
        if ($this->Session->check('Auth.User')) {
            ...
            $this->User->updateLastVisit($this->Session->read('Auth.Affiliate.user_id'));
            //Cache::write('user_session_id_' . $this->Auth->user('id'), session_id(), 'longterm');
            /* log */
            $dd['UserLog']['user_id'] = $this->Session->read('Auth.Affiliate.user_id');
            $dd['UserLog']['action'] = 'login';
            $dd['UserLog']['date'] = $this->__getSqlDate();
            $dd['UserLog']['ip'] = $this->RequestHandler->getClientIP();
...
//        $this->set('groups', $groups);
//
//        $this->log('AFFILIATE LOGIN REQUEST');
//
...
//            $this->log('LOGGED IN AFFILIATE');
//            $this->log($this->Session->read('Auth.User'));
//            $this->log($this->request->data['User']['group_id']);
//
//            if ($this->Session->read('Auth.User.group_id') != $this->request->data['User']['group_id']) {
//                var_dump('ERROR');
//                $this->__setError(__('Username or password is incorrect!'), 'default', array(), 'auth');
...
//            }
//            $this->_loadPermissions();
//            $this->log('CHECK LOGGED IN AFFILIATE');
//            $this->log($this->Session->check('Auth.User'));
//
//            if ($this->Session->check('Auth.User')) {
//
//                $this->User->updateLoginStatus($this->Auth->user('id'));
...
//    }
    ...
    public function affiliate_logout() {
        ...
        if (CakeSession::read('Auth.Affiliate.id')) {
            ...
            $this->User->updateLogout(CakeSession::read('Auth.Affiliate.user_id'));
            $dd['UserLog']['user_id'] = CakeSession::read('Auth.Affiliate.user_id');
...
//        $groupId = $this->Auth->user('group_id');
    ...
    public function admin_index($conditions = array(), $model = NULL) {
        ...
        if (!empty($this->request->data)) {
            foreach ($this->request->data['User'] as $key => $value) {
                ...
                if ($key == 'balance_from') {
                    $conditions[] = array('User.balance >=' => $value);
                    continue;
                }
                ...
                if ($search_elements['User.' . $key]['type'] == 'text' && strpos($value, "*") !== FALSE) {
                    //var_dump($search_elements['User.' . $key]['type']);
        ...
        //$conditions = array('User.id' => array(777, 1023, 1064, 1068, 1069));
        // displayed fields
        ...
        foreach ($data as &$userdata) {
            ...
            $KYC = $this->KYC->find('all', array('conditions' => array('user_id' => $userdata['User']['id']), 'recursive' => -1));

            $affiliate = $this->Affiliate->find('first', array(
                'recursive' => -1,
                'conditions' => array('Affiliate.id' => $userdata['User']['affiliate_id']),
            ));
            ...
            $userdata['User']['kyc'] = $KYC;


            if (!empty($affiliate))
                $userdata['User']['affiliate'] = $affiliate;

            //var_dump(count($userdata['UserSettings']));
            ...
            $user = $this->User->getItem($userdata['User']['id'], -1, array('Currency', 'Country'));

            $userdata['User']['Currency'] = $user['Currency'];
            $userdata['User']['Country'] = $user['Country'];
        }
        //var_dump($data);
    ...
    public function admin_add($id = null) {
        ...
        $this->set('user_categories', $this->UserCategory->list_categories());
        $this->set('countries', $this->Country->list_countries());
        $this->set('currencies', $this->Currency->list_currencies());
    }

    function admin_edit($user_id, $view = NULL) {
        ...
        if (!empty($this->request->data)) {
            ...
            $country = $this->Country->find('first', array('conditions' => array('Country.id' => $this->request->data['User']['country_id'])));

            $this->request->data['User']['country'] = $country['Country']['alpha2_code'];
...
//            }
        ...
        parent::admin_edit($user_id, 'view');
...
//        }
        ...
        $this->set('currencies', $this->Currency->list_currencies());
        $this->set('user', $user);
        $this->set('affiliates', $this->Raventrack->find('all', array('recursive' => -1)));
    }
    ...
    public function admin_kyc($id) {
        ...
        if (!empty($this->request->data)) {
            ...
            $user['User']['kyc_valid_until'] = $this->request->data['User']['created'];
     ...
     */
    public function admin_lgalimits($user_id) {
        $limits['deposit'] = $this->UsersLimits->getUserLimits($user_id, "deposit");
        $limits['wager'] = $this->UsersLimits->getUserLimits($user_id, "wager");
        $limits['loss'] = $this->UsersLimits->getUserLimits($user_id, "loss");
        $limits['sessionlimit'] = $this->UsersLimits->getUserLimits($user_id, "session");
        $limits['selfexclusion'] = $this->UsersLimits->getUserLimits($user_id, "selfexclusion");
        $limits['deleteaccount'] = $this->UsersLimits->getUserLimits($user_id, "deleteaccount");
        //var_dump($limits['selfexclusion']);
        $fields['User']['id'] = $user_id;
        $this->set(compact('fields', 'limits'));
    }
     ...
     * Admin User Notes
     */
    public function admin_add_notes() {
        $this->autoRender = false;
        if (!empty($this->request->data)) {
            $this->Note->add_note($this->request->data['user_id'], $this->request->data['note']);
        }
    }

    /**
     * Admin User Notes with Couch DB//OLD
     */
    ...
    public function playerPaymentsStatistics($user_id) {

        try {

            $user = $this->User->getItem($user_id, 0);

            $statistics = $this->Payment->find('all', array('conditions' => array('user_id' => $user_id),
                'fields' => array(
                    'COALESCE(SUM(Payment.amount),0) AS sum, COALESCE(COUNT(*), 0) AS count, Payment.status as status, Payment.type as type'
                ), 'group' => array('Payment.type', 'Payment.status')));


            $response = array('status' => 'success', 'data' => $statistics, 'currency' => $user['Currency']['code']);
        } catch (Exception $ex) {
    ...
    public function playerCasinoStatistics($user_id) {

        try {

            $user = $this->User->getItem($user_id, 0);

            $casino_transactions = $this->TransactionLog->find('all', array('conditions' => array('user_id' => $user_id, 'model' => 'Games'),
                'fields' => array(
                    'COALESCE(SUM(TransactionLog.amount),0) AS sum, COALESCE(COUNT(*), 0) AS count, TransactionLog.transaction_type as type'
                ), 'group' => array('TransactionLog.transaction_type')));


            $games_statistics = $this->IntGameActivity->find('all', array('conditions' => array('user_id' => $user_id),
                'fields' => array(
                    'COALESCE(COUNT(DISTINCT int_game_id), 0) AS total'
                ), 'recursive' => 0
            ));

            $response = array('status' => 'success', 'casino_transactions' => $casino_transactions, 'games_statistics' => $games_statistics, 'currency' => $user['Currency']['code']);
        } catch (Exception $ex) {
            $response = array('status' => 'error', 'message' => $ex->getMessage());
        }
        return $response;
    ...
    public function admin_view($id) {
        ...
        $user_data = $this->User->getItem($id, 0);
        if (!empty($user_data)) {
...
//            $user_data['User']['bank_code']         = Security::rijndael($user_data['User']['bank_code'], Configure::read('Security.rijndaelkey'), 'decrypt');
        ...
        $this->loadModel('KYC');
        $user_data['User']['kyc_status'] = KYC::$humanizeStatuses[$user_data['User']['kyc_status']];

        //$user_data['User']['account_number'] = '***';
        //$user_data['User']['bank_code'] = '***';

        /* FLASH MESSAGE IF USER IS UNDER RISK MANAGEMENT SETTINGS */
        if ($this->UserSettings->hasSettings($id))
            $this->__setError(__("User Under Custom Risk Management Settings"));

        /* NOTES PER USER */
        //add  a different functionality for adding nots
        //
        $this->loadModel('Note');
        $notes = $this->Note->find('all', array('conditions' => array('user_id' => $id), 'recursive' => 0, 'order' => array('Note.created' => 'DESC')));
        ...
        //Payments
        //Payments
        $deposits = $this->Payment->query("select * from payments as Payment"
                . " INNER JOIN users as User ON Payment.user_id = User.id"
                . " INNER JOIN currencies as Currency ON User.currency_id = Currency.id"
                . "  WHERE Payment.user_id=" . $id . " AND Payment.type='Deposit' order by Payment.created desc LIMIT 20");
        $this->set('deposits', $deposits);


        $withdraws = $this->Payment->query("select * from payments as Payment"
                . " INNER JOIN users as User ON Payment.user_id = User.id"
                . " INNER JOIN currencies as Currency ON User.currency_id = Currency.id"
                . "  WHERE Payment.user_id=" . $id . " AND Payment.type='Withdraw' order by Payment.created desc LIMIT 20");
        $this->set('withdraws', $withdraws);



        //Statistics
        $this->set('payment_statistics', $this->playerPaymentsStatistics($id));
        //$this->set('casino_statistics', $this->playerCasinoStatistics($id));
//        $this->loadModel('Payments.Payment');
//        $deposits = $this->Payment->query("select * from payments WHERE user_id=" . $id . " order by created desc LIMIT 20");
//        $this->set('deposits', $deposits);
    ...
    public function admin_request_kycdoc($user_id) {
        $this->autoRender = false;
        $user = $this->User->getItem($user_id);
        ...
        if (!empty($user)) {
            ...
            //$this->__sendMail('kyc_request', $user['User']['email'], $vars);

            if ($this->__sendMail('kyc_request', $user['User']['email'], $vars)) {
                ...
                $this->__setError(__('Cannot send email.', true));
            }
        }
    }
    ...
    public function signUp() {
        ...
        try {
            ...
            if (!empty($userdata['affiliate_code'])) {
                $affiliate = $this->Affiliate->find('first', array('conditions' => array('referral_id' => strtoupper($userdata['affiliate_code']))));
                $this->log($affiliate);
            }
            ...
            $user['User'] = array(
                ...
                'login_status' => User::USER_LOGGED_OUT,
                'last_visit' => $this->__getSqlDate(),
                'last_visit_ip' => $this->RequestHandler->getClientIP(),
                'bonus_allow' => ($userdata['bonus_allow'] ? 1 : 0),
                    //'turkish_identity' => $userdata['turkish_identity']
            );
...
//                $user['User']['landing_page'] = $this->Session->read('landing');
            ...
            if ($savedUser = $this->User->save($user)) {
                /*
                 * ADD USER DATA TO CUSTOMER IO
                 */
                $customer = $this->User->getUser($savedUser['User']['id']);
                unset($customer['password']);
                unset($customer['confirmation_code']);
                unset($customer['confirmation_code_created']);
//                $customer['currency'] = $userdata['currency'];
//                $customer['language'] = $userdata['language'];
                $customer['User']['registration_device'] = $_SERVER['HTTP_USER_AGENT'];

                $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterRegister', $this, array('user_id' => $savedUser['User']['id'])));

                $this->log('REGISTER', 'CustomerIO');
                $this->log($customer, 'CustomerIO');
                $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomer', $this, array('customer' => $customer, 'update' => false)));

                //add device to Customer IO
                $device_id = base64_encode($_SERVER['HTTP_USER_AGENT']);
                $platform = $this->detectUserAgent();
                if ($platform !== false)
                    $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomerDevice', $this, array('customer' => $customer, 'device_id' => $device_id, 'platform' => $platform)));



                if (isset($userdata['query_params']['btag'])) {
                    $query_params = $userdata['query_params'];

                    $savedUser['User']['affiliate_id'] = isset($query_params['affid']) ? $query_params['affid'] : NULL;
                    $savedUser['User']['landing_page'] = isset($query_params['lpage']) ? $query_params['lpage'] : NULL;
                    $savedUser['User']['btag'] = isset($query_params['btag']) ? $query_params['btag'] : NULL;

                    $this->User->save($savedUser);

                    //add Raventrack data
                    $raventrack_referral['Referral'] = array(
                        "user_id" => $savedUser['User']['id'],
                        "affiliate_id" => isset($query_params['affid']) ? $query_params['affid'] : NULL,
                        "btag" => isset($query_params['btag']) ? $query_params['btag'] : NULL,
                        "landing_page" => isset($query_params['lpage']) ? $query_params['lpage'] : NULL,
                        "s1" => isset($query_params['s1']) ? $query_params['s1'] : NULL,
                        "s2" => isset($query_params['s2']) ? $query_params['s2'] : NULL,
                        "s3" => isset($query_params['s3']) ? $query_params['s3'] : NULL,
                        "click" => isset($query_params['click']) ? $query_params['click'] : NULL,
                        "campaign" => isset($query_params['campaign']) ? $query_params['campaign'] : NULL,
                        "gclid" => isset($query_params['gclid']) ? $query_params['gclid'] : NULL,
                        "msclkid" => isset($query_params['msclkid']) ? $query_params['msclkid'] : NULL,
                        "resource" => isset($query_params['resource']) ? $query_params['resource'] : NULL,
                        "site" => isset($query_params['site']) ? $query_params['site'] : NULL,
                    );
                    $this->log('RAVENTRACK REFERRAL', 'Raventrack');
                    $this->log($raventrack_referral, 'Raventrack');
                    $this->Referral->save($raventrack_referral);
                }
                ...
                $response = array('status' => 'success', 'message' => __('Account has been created. We have sent a verification link to your e-mail. In order to activate your account please, verify your e-mail. If you do not see a message in your inbox, make sure the email address you provided on sign up is correct and check your junk and spam mail folders too.', true), 'error' => '');
            } else {
    ...
    public function signIn() {
        ...
        //$this->log('Player Sign In');
        //$this->log($this->request);
        if ($this->request->isPost()) {
            if ($this->Auth->login()) {
                $this->log('Player Sign In AUTH');
                $this->log($this->Auth->user());

                if ($this->Auth->user('status') == User::USER_STATUS_BANNED) {  //Account Banned
                    $this->Auth->logout();
                    return json_encode(array('status' => 'error', 'message' => __('Your account has been suspended. Please contact our support at support@atlanticslot.com in order to resolve the issue.'), 'errredirect' => ''));
                }

                if ($this->Auth->user('status') == User::USER_STATUS_UNCONFIRMED) {  //Confirm email
                    $this->Auth->logout();
                    return json_encode(array('status' => 'error', 'message' => __('Please confirm your email in order to sign in.'), 'errredirect' => ''));
                }

                if ($this->Auth->user('status') == User::USER_STATUS_LOCKEDOUT) { //Lockedout
                    $this->Auth->logout();
                    return json_encode(array('status' => 'error', 'message' => __('Your account is locked. Please reset your password in order to sign in.'), 'errredirect' => '/#/recovery/newpass'));
                }
                ...
                if ($this->Auth->user('login_status') == 1 &&
                    ...
                    $this->log('user is logged out from other devices when login to new device');

                    $this->log($this->Auth->user('last_visit_sessionkey'));
                    $this->log($this->Auth->user('last_visit_sessionkey'));
                    $this->log($this->User->timediff_user('Now', $this->Auth->user('last_visit')));
...
//                $language = $this->Language->findById($this->Auth->user('language_id'));
...
//                Configure::write('Config.language', $language['Language']['locale_code']);
                //$this->User->checkLastVisit($this->Auth->user('id'), $this->RequestHandler->getClientIP());
                ...
                /*
                 * ADD LAST LOGIN DATA TO CUSTOMER IO
                 */

                $customer = $this->User->getUser($this->Auth->user('id'));
                $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomer', $this, array('customer' => $customer, 'update' => true)));
                //add update device to Customer IO
                $device_id = base64_encode($_SERVER['HTTP_USER_AGENT']);
                $platform = $this->detectUserAgent();
                if ($platform !== false)
                    $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomerDevice', $this, array('customer' => $customer, 'device_id' => $device_id, 'platform' => $platform)));

                $response = array('status' => 'success', 'data' => CakeSession::read('Auth.User')); //$this->Auth->user()
            } else {  //if not authenticated
                $fail_counter = $this->User->updateFailedLogin($this->request->data['User']['username']);
                $response = array('status' => 'error', 'message' => __('Wrong username or password!'), 'errredirect' => '');
                if ($fail_counter > 3) {

                    $datafailuser = $this->User->find('first', array('conditions' => array('username' => $this->request->data['User']['username']), 'recursive' => 0));

                    /*
                     * UPDATE FAILED COUNTER IN CUSTOMER IO
                     */
                    $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomer', $this, array('customer' => $datafailuser, 'update' => true)));

                    /*
                     * ADD PLAYER LOCKED ACCOUNT TO CUSTOMER IO EVENTS
                     */
                    //$event->data['customer']['id'], $event->data['event']['name'], $event->data['event']['type'], $event->data['data'], $event->data['event']['recipient'], $event->data['event']['from_address'], $event->data['event']['reply_to']
                    $event = array(
                        'name' => 'player_locked_account',
                        'type' => 'event',
                        'recipient' => null,
                        'from_address' => null,
                        'reply_to' => null
                    );
                    $this->log('LOCKED ACCOUNT', 'CustomerIO');
                    $this->log($datafailuser, 'CustomerIO');
                    $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterTrackCustomerEvent', $this, array('customer' => $datafailuser, 'data' => $datafailuser['User'], 'event' => $event)));

                    $this->User->lockAccount($datafailuser['User']['id']);
    ...
    public function signOut() {
        ...
        if ($this->Auth->user('id')) {
            $this->Session->write('user.loggedout', 1);
            $this->Session->delete('loginfor');
            $this->User->updateLogout($this->Auth->user('id'), $this->RequestHandler->getClientIP(), $_SERVER['HTTP_USER_AGENT']);
            $dd['UserLog']['user_id'] = $this->Auth->user('id');
            $dd['UserLog']['action'] = 'logout';
            $dd['UserLog']['date'] = $this->__getSqlDate();
            $dd['UserLog']['ip'] = $this->RequestHandler->getClientIP();
            ...
            $customer = $this->User->getUser($this->Auth->user('id'));
            $this->log('SIGN OUT', 'CustomerIO');
            $this->log($customer, 'CustomerIO');
            $this->getEventManager()->dispatch(new CakeEvent('Model.User.afterAddUpdateCustomer', $this, array('customer' => $customer, 'update' => true)));
        }
        return $this->Auth->logout();
        //return $this->redirect($this->Auth->logout(), 302, true);
        //$this->redirect(array('controller' => 'users', 'action' => 'signOut'));
    }
    ...
    public function affiliate_index() {
        ...
        if (!empty($this->request->data)) {
            foreach ($this->request->data['User'] as $key => $value) {
                ...
                $conditions['User.affiliate_id'] = $this->Session->read('Auth.Affiliate.id');
            }
        } else {
        ...
        // actions
        $this->set('actions', $this->User->getAffiliateActions());
...
//        var_dump($conditions);
        //var_dump($conditions);
        $data = $this->paginate(array('User.affiliate_id' => $this->Session->read('Auth.Affiliate.id')));
    ...
    public function affiliate_view($id) {
        ...
        $currency = $this->Currency->find('first', array('conditions' => array('name' => $user_data['User']['currency_id'])));

        $user_data['User']['currency'] = $currency;
        $user_data['User']['status'] = User::$User_Statuses_Humanized[$user_data['User']['status']];
        $user_data['User']['login_status'] = User::$user_login_statuses[$user_data['User']['login_status']];
        ...
        $this->loadModel('Payments.Payment');
        $deposits = $this->Payment->query("select * from payments WHERE user_id=" . $id . " AND status = 'Completed' order by created desc");
        $this->set('deposits', $deposits);
