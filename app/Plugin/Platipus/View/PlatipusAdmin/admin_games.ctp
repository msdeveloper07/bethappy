            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        ...
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'platipus', 'controller' => 'platipus_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'platipus', 'controller' => 'platipus_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>

                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'platipus', 'controller' => 'platipus_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'platipus', 'controller' => 'platipus_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['PlatipusGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['PlatipusGames']['id']; ?>"><?= $row['PlatipusGames']['id']; ?></td>
                                                        <td><?= $row['PlatipusGames']['game_id']; ?></td>
                                                        <td><?= $row['PlatipusGames']['game_key']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['PlatipusGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['PlatipusGames']['category'] ?>" /></td>
                                                        <td><?= $row['PlatipusGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['PlatipusGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['PlatipusGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['PlatipusGames']['image'] ?>" data-image="<?= $row['PlatipusGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['PlatipusGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['PlatipusGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['PlatipusGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['PlatipusGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['PlatipusGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['PlatipusGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['PlatipusGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['PlatipusGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/platipus/platipus_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
