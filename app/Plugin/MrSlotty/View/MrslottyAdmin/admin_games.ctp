            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        ...
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'mrslotty', 'controller' => 'mrslotty_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'mrslotty', 'controller' => 'habanero_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>

                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'mrslotty', 'controller' => 'mrslotty_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'mrslotty', 'controller' => 'mrslotty_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['MrslottyGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['MrslottyGames']['id']; ?>"><?= $row['MrslottyGames']['id']; ?></td>
                                                        <td><?= $row['MrslottyGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['MrslottyGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['MrslottyGames']['category'] ?>" /></td>
                                                        <td><?= $row['MrslottyGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['MrslottyGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['MrslottyGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['MrslottyGames']['image'] ?>" data-image="<?= $row['MrslottyGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['MrslottyGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['MrslottyGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['MrslottyGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['MrslottyGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['MrslottyGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['MrslottyGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['MrslottyGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['MrslottyGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/mrslotty/mrslotty_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
