            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'playson', 'controller' => 'playson_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'playson', 'controller' => 'playson_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'playson', 'controller' => 'playson_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'playson', 'controller' => 'playson_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                        </div>
                        <br>
                        <div class="row-fluid">
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['PlaysonGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['PlaysonGames']['id']; ?>"><?= $row['PlaysonGames']['id']; ?></td>
                                                        <td><?= $row['PlaysonGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['PlaysonGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['PlaysonGames']['category'] ?>" /></td>
                                                        <td><?= $row['PlaysonGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['PlaysonGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['PlaysonGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['PlaysonGames']['image'] ?>" data-image="<?= $row['PlaysonGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['PlaysonGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['PlaysonGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['PlaysonGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['PlaysonGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['PlaysonGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['PlaysonGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['PlaysonGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['PlaysonGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/playson/playson_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
