...
class PlaysonGuid extends PlaysonAppModel {

    /**
     * Model name
     * @var string
     */
    public $name = 'PlaysonGuid';
...
//    }
    ...
    public function closeGuid($guid) {
        ...
        $playsonGuid['PlaysonGuid']['logouttime'] = strtotime("NOW");
    ...
    public function getGuid($guid) {
        ...
        return $this->getUser($playsonGuid['PlaysonGuid']['sessionid']);
    }
    ...
    public function userGuid($options) {
        $transactions = $this->query("
            ...
            from Playsonguid as guid 
            inner join PlaysonLogs as logs on logs.guid = guid.id 
            inner join users as u on u.id = guid.user_id where 1=1"
                ...
                . (!empty($options['game_code']) ? " and guid.game_id = '{$options['game_code']}'" : "")
        );
