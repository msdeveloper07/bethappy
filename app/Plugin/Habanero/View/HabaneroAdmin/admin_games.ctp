            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'habanero', 'controller' => 'habanero_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'habanero', 'controller' => 'habanero_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'habanero', 'controller' => 'habanero_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'habanero', 'controller' => 'habanero_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['HabaneroGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['HabaneroGames']['id']; ?>"><?= $row['HabaneroGames']['id']; ?></td>
                                                        <td><?= $row['HabaneroGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['HabaneroGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['HabaneroGames']['category_id'] ?>" /></td>
                                                        <td><?= $row['HabaneroGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['HabaneroGames']['pay_lines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['HabaneroGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['HabaneroGames']['image'] ?>" data-image="<?= $row['HabaneroGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['HabaneroGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['HabaneroGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkjackpot" type="checkbox" <?= ($row['HabaneroGames']['jackpot'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['HabaneroGames']['free_spins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['HabaneroGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['HabaneroGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['HabaneroGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['HabaneroGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['HabaneroGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/habanero/habanero_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
