            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'igromat', 'controller' => 'igromat_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'igromat', 'controller' => 'igromat_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'igromat', 'controller' => 'igromat_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'igromat', 'controller' => 'igromat_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                        </div>
                        <br>
                        <div class="row-fluid">
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['IgromatGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['IgromatGames']['id']; ?>"><?= $row['IgromatGames']['id']; ?></td>
                                                        <td><?= $row['IgromatGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['IgromatGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['IgromatGames']['category'] ?>" /></td>
                                                        <td><?= $row['IgromatGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['IgromatGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['IgromatGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['IgromatGames']['image'] ?>" data-image="<?= $row['IgromatGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['IgromatGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['IgromatGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['IgromatGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['IgromatGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['IgromatGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['IgromatGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['IgromatGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['IgromatGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/igromat/igromat_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
