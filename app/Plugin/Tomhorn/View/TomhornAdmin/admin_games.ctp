            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'tomhorn', 'controller' => 'tomhorn_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'tomhorn', 'controller' => 'tomhorn_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'tomhorn', 'controller' => 'tomhorn_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'tomhorn', 'controller' => 'tomhorn_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                        </div>  <br>
                        <div class="row-fluid">
                            <div class="span12">
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['TomhornGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['TomhornGames']['id']; ?>"><?= $row['TomhornGames']['id']; ?></td>
                                                        <td><?= $row['TomhornGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['TomhornGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['TomhornGames']['category'] ?>" /></td>
                                                        <td><?= $row['TomhornGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['TomhornGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['TomhornGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['TomhornGames']['image'] ?>" data-image="<?= $row['TomhornGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['TomhornGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['TomhornGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['TomhornGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['TomhornGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['TomhornGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['TomhornGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['TomhornGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['TomhornGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/tomhorn/tomhorn_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
