            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'betsoft', 'controller' => 'betsoft_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'betsoft', 'controller' => 'betsoft_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'betsoft', 'controller' => 'betsoft_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'betsoft', 'controller' => 'betsoft_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                        </div> <br>
                        <div class="row-fluid">
                            <div class="span12">
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['BetsoftGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['BetsoftGames']['id']; ?>"><?= $row['BetsoftGames']['id']; ?></td>
                                                        <td><?= $row['BetsoftGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['BetsoftGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['BetsoftGames']['category'] ?>" /></td>
                                                        <td><?= $row['BetsoftGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['BetsoftGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['BetsoftGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['BetsoftGames']['image'] ?>" data-image="<?= $row['BetsoftGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['BetsoftGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['BetsoftGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['BetsoftGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['BetsoftGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['BetsoftGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['BetsoftGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['BetsoftGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['BetsoftGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/betsoft/betsoft_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
