            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>
                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                        ...
                        <div class="row-fluid">
                            <div class="span12">
    <!--<?//= $this->Html->link(__('Download new'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>-->
    <!--<?//= $this->Html->link(__('Sync new'), array('plugin' => 'netent', 'controller' => 'netent_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>-->
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['NetentGames']['id']; ?>">

                                                        <td class="popper" data-popbox="pop1_<?= $row['NetentGames']['id']; ?>"><?= $row['NetentGames']['id']; ?></td>
                                                        <td><?= $row['NetentGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['NetentGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['NetentGames']['category'] ?>" /></td>
                                                        <td><?= $row['NetentGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['NetentGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['NetentGames']['reels'] ?>" /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['NetentGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['NetentGames']['image'] ?>" data-image="<?= $row['NetentGames']['image'] ?>" /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['NetentGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['NetentGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['NetentGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['NetentGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['NetentGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/netent/netent_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
                        '&reels=' + _reels + '&freespins=' + _freespins + '&mobile=' + _mobile + '&desktop=' + _desktop + '&image=' + _img +
