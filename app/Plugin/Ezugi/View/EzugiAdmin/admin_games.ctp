            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        ...
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'ezugi', 'controller' => 'ezugi_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'ezugi', 'controller' => 'ezugi_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>

                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'ezugi', 'controller' => 'ezugi_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'ezugi', 'controller' => 'ezugi_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['EzugiGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['EzugiGames']['id']; ?>"><?= $row['EzugiGames']['id']; ?></td>
                                                        <td><?= $row['EzugiGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['EzugiGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['EzugiGames']['category'] ?>" /></td>
                                                        <td><?= $row['EzugiGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['EzugiGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['EzugiGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['EzugiGames']['image'] ?>" data-image="<?= $row['EzugiGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['EzugiGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['EzugiGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['EzugiGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['EzugiGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['EzugiGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['EzugiGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['EzugiGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['EzugiGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/ezugi/ezugi_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
