...
class MicrogamingGames extends MicrogamingAppModel {

    /**
     * Model name
     * @var type 
     */
    public $name = 'MicrogamingGames';

    /**
     * Database table name
     * @var type 
     */
    public $useTable = 'MicrogamingGames';
    ...
    public function gameExists($id, $name) {
        $options['conditions'] = array('AND' => array('MicrogamingGames.game_id' => $id, 'MicrogamingGames.name' => $name));
    ...
    public function enableGame($id) {
        $options['conditions'] = array('MicrogamingGames.id' => $id);

        $game = $this->find('first', $options);

        if (!empty($game)) {
            $game['MicrogamingGames']['active'] = 1;
            if ($this->save($game)) {
                return true;
            } else {
                return false;
    ...
    public function disableGame($id) {
        ...
        if (!empty($game)) {
            $game['MicrogamingGames']['active'] = 0;
            if ($this->save($game)) {
                return true;
            } else {
                return false;
    ...
    public function disableGames() {
        try {
            ...
            $result = $this->query('UPDATE `MicrogamingGames` SET `active` = 0;');
            if ($result)
                return true;
...
//    }
    ...
    public function getGameUrl($game_id, $funplay) {
        ...
        if ($game_id) {
            ...
            $url = "/microgaming/microgaming/game/" . $game_id . "/" . $funplay . "/" . $user_id;
