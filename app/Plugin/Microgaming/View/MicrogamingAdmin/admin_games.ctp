            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'microgaming', 'controller' => 'microgaming_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'microgaming', 'controller' => 'microgaming_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>

                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'microgaming', 'controller' => 'microgaming_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'microgaming', 'controller' => 'microgaming_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['MicrogamingGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['MicrogamingGames']['id']; ?>"><?= $row['MicrogamingGames']['id']; ?></td>
                                                        <td><?= $row['MicrogamingGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['MicrogamingGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['MicrogamingGames']['category'] ?>" /></td>
                                                        <td><?= $row['MicrogamingGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['MicrogamingGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['MicrogamingGames']['reels'] ?>" /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['MicrogamingGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['MicrogamingGames']['image'] ?>" data-image="<?= $row['MicrogamingGames']['image'] ?>" /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['MicrogamingGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['MicrogamingGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['MicrogamingGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['MicrogamingGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['MicrogamingGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/microgaming/microgaming_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
