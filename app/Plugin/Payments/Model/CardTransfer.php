 ...
 *
 * Handles CardTransfer payment gateway data
 *
 * @package    Payments
 * @author     
 * @copyright  
...
class CardTransfer extends PaymentAppModel {
     ...
     * @var string
     */
    public $table = 'payments_CardTransfer';

    /**
     * Custom database table name, or null/false if no table association is desired.
     * @var string
     */
    public $useTable = 'payments_CardTransfer';
    ...
    public function __construct($id = false, $table = null, $ds = null) {
        ...
        Configure::load('Payments.CardTransfer');

        if (Configure::read('CardTransfer.Config') == 0)
            throw new Exception('Config not found', 500);

        $this->config = Configure::read('CardTransfer.Config');
    }
    ...
    public function prepare_transaction($transaction_data) {
        try {
            ...
            $this->log('CARD TRANSFER WITHDRAW REQUEST', 'Withdraws');
    ...
    public function approve_withdraw($transaction_id) {
        ...
        try {
            $transaction = $this->find('first', array('conditions' => array('CardTransfer.id' => $transaction_id), 'recursive' => -1));

            if ($transaction['CardTransfer']['status'] == self::TRANSACTION_PENDING) {
                ...
                $transaction['CardTransfer']['status'] = self::TRANSACTION_COMPLETED;
                $transaction['CardTransfer']['logs'] = $transaction['CardTransfer']['logs'] . "\n\r" . "Transaction updated with status: " . self::TRANSACTION_COMPLETED . " on " . $this->getSqlDate();
    ...
    public function cancel_withdraw($transaction_id) {
        try {
            ...
            if ($transaction['CardTransfer']['status'] == self::TRANSACTION_PENDING) {
                ...
                $payment = $this->Payment->find('first', array('conditions' => array('Payment.provider' => $this->name, 'Payment.parent_id' => $transaction_id, 'Payment.user_id' => $transaction[$this->name]['user_id'])));
                $transaction['CardTransfer']['status'] = self::TRANSACTION_CANCELLED;
                ...
                $this->User->addFunds($transaction['CardTransfer']['user_id'], 'Payments', $this->name, 'Refund', $transaction['CardTransfer']['amount'], $payment['Payment']['id']);
     ...
     */
     public function getSearch() {
        ...
        return array(
            'CardTransfer.id' => array('type' => 'text', 'label' => __('ID'), 'class'=>'form-control'),
            'CardTransfer.user_id' => array('type' => 'number', 'label' => __('User ID'), 'class'=>'form-control'),
            'User.username' => array('type' => 'text', 'label' => __('Username'), 'class'=>'form-control'),
            'CardTransfer.amount_from' => $this->getFieldHtmlConfig('number', array('label' => __('Amount from'), 'id' => 'amount_from')),
            'CardTransfer.amount_to' => $this->getFieldHtmlConfig('number', array('label' => __('Amount to'))),
            'CardTransfer.date_from' => $this->getFieldHtmlConfig('date', array('label' => __('Deposit Date From'), 'id' => 'date_from')),
            'CardTransfer.date_to' => $this->getFieldHtmlConfig('date', array('label' => __('Deposit Date To'), 'id' => 'date_to')),
            'CardTransfer.remote_id' => array('type' => 'text', 'label' => __('Remote ID'), 'class'=>'form-control'),
            'CardTransfer.status' => $this->getFieldHtmlConfig('select', array('label' => __('Status'), 'options' => $statuses)),
