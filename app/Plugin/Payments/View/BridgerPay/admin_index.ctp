        ...
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?= __('Bridger Pay'); ?></li>
                </ol>
            </nav>

            <div class="d-flex justify-content-between">
                <h1 class="h3 mb-1 text-gray-800"><?= __('Bridger Pay'); ?></h1>
            </div>
            <br>
        </div>
    </div>
    ...
    <div id="page" class="dashboard">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <?php
                echo $this->Form->create(false, array('url' => "/admin/payments/BridgerPay", 'type' => 'file', 'id' => 'search-form', 'action' => 'index'));

                foreach ($search_fields AS $i => &$field) {

                    $kk = str_replace("BridgerPay.", "", $i);
                    $field['value'] = $search_values['BridgerPay'][$kk];
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                <?php foreach ($data as $row): ?>
                                    <tr>
                                        <td><a class="paymentinfo" data-id="<?= $row['BridgerPay']['id']; ?>" data-popbox="pop1" data-content="<?= nl2br($row['BridgerPay']['logs']); ?>"><?= $row['BridgerPay']['id']; ?></a></td>
                                        ...
                                        <td><?= $row['BridgerPay']['amount'] . $row['User']['Currency']['code']; ?></td>
                                        <td><?= $row['BridgerPay']['currency']; ?></td>
                                        ...
                                        <td><?= $row['BridgerPay']['remote_id']; ?></td>
                                        <td><?= $row['BridgerPay']['ip']; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($row['BridgerPay']['date'])); ?></td>
                                        <td><?= $row['BridgerPay']['error_mesage']; ?></td>
                                        <td>
                                            <?= $this->element('status_button', array('status' => __(array_search($row['BridgerPay']['status'], BridgerPay::$humanizeStatuses)))); ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
