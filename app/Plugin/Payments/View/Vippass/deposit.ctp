			...
			<div class="col-12">
				<div class="d-flex align-items-center justify-content-center">
					<div class="d-flex align-items-center">
						...
						<div class="ml-3">
							<span class="deposit-method-name mb-0_25" id="cName">VIPPASS</span>
...
<script>
	...
	function requestDeposit(event) {
		...
		$.ajax({        
			url: `/payments/vippass/deposit/${amount}`,
			type : 'POST',
			data : {
				address1: address1,
				address2: address2,
			...
			success : function(data) {  
				...
				if (data.status === 'Limit Error') {
					swal("<?=__("Error")?>", data.information, "error");
					return;
				}
				
				window.open(data.THPPRedirectURL, '_blank', 'location=yes,height=570,width=800,scrollbars=yes,status=yes');
