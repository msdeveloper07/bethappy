        ...
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?= __('Deposits Reports'); ?></li>
                </ol>
            </nav>

            <div class="d-flex justify-content-between">
                <h1 class="h3 mb-1 text-gray-800"><?= __('Deposits Reports'); ?></h1>
            </div>
            <br>
        </div>
    </div>
                    ...
                    <?php if (!empty($data)): ?>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                ...
                                <tr>
                                    <td colspan="" style="border-right: none;">
                                        <strong><?= $provider ? $provider . ' ' . __('Deposits Report') : __('Deposits Report'); ?></strong>
                                    </td>
                                    <td colspan="<?= empty($selected_provider) ? 5 : 4 ?>"  style="text-align:right; border-left: none;">
                                ...
                                foreach ($data as $currency => $transactions):
                                    ...
                                    foreach ($transactions as $key => $transaction):
                                        ...
                                        <tr class="transaction-body">
                                            ...
                                            <?php endif; ?>
                                            <td colspan="2" style="text-align: center; "><?= ucwords(str_replace("'", "", $transaction['depositor_name'])); ?></td>
                        ...
                        <div class="print-form text-center mb-4">
                            ...
                            <input type='hidden' name='data[htmldata]' value='<?= $this->fetch('htmldata'); ?>'/>
                            <input type='hidden' name='data[Report][header]' value='<?= $provider ? $provider . ' ' : '' . __('Deposits Report'); ?>'/>
                            <input type='hidden' name='data[Report][title]' value='<?= $provider ? $provider . '_' : '' . __('Deposits_Report'); ?>'/>
...
<!--<?//= $this->element('status_button', array('status' => $transaction['status'])); ?>-->
<!--<?//= ucwords(str_replace("'", "", $transaction['depositor_name']));?>-->