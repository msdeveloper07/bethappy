...
<form id="astropay-form" method="post" onsubmit="requestWithdraw(event);">
				...
				<div class="col-12 col-lg-6 offset-lg-3">
					<div class="p-2 p-lg-4 d-flex justify-content-center flex-column">
						...
						<div class="form-group w-100">
							<label class="control-label"><?=__('Withdraw Sum')?> (<?=$user['Currency']['code']?>)</label>
                        ...
                        <div class="form-group mt-2">
                            <label class="control-label"><?=__("Choose a currency")?></label>
                        ...
                        <div class="form-group mt-2">
                            <label class="control-label"><?=__("Enter crypto currency address")?></label>
                            <input type="text" class="form-control" id="address" required />
                        </div>
                        
                        <div class="mt-4">
                            <button id="btn-pay" type="submit" class="btn-default btn-submit w-100"><?=('Pay Out')?></button>
                        </div>
					</div>
				</div>
...
<script>
    ...
    function requestWithdraw(event) {
        ...
        let cryptoCurrency = $("#currency-select").val();
        let address = $("#address").val();

        let url = `/payments/ForumPay/startPaymentOut/${cryptoCurrency}/${amount}/${address}`;
        $.ajax({
            url: url,
            dataType:'json',
            success: function(result) {
                if (result.err) {
                    ...
                    swal("<?=__("Accepted")?>", result.message, "success");
                }

                // window.location = `/payments/ForumPay/trackPayOut/${result.id}`;
                // console.log(result);
            }
        });         
    }
