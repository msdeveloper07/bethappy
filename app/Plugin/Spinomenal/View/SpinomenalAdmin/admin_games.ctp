            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-body">
                        ...
                        <div class="row-fluid">
                            <div class="pull-left">
                                <?= $this->Html->link(__('Get New'), array('plugin' => 'spinomenal', 'controller' => 'spinomenal_admin', 'action' => 'getNewGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync New'), array('plugin' => 'spinomenal', 'controller' => 'spinomenal_admin', 'action' => 'syncNewGames'), array('class' => 'btn btn-primary')); ?>

                            </div>
                            <div class="pull-right">
                                <?= $this->Html->link(__('Update All'), array('plugin' => 'spinomenal', 'controller' => 'spinomenal_admin', 'action' => 'updateGames'), array('class' => 'btn btn-primary')); ?>
                                <?= $this->Html->link(__('Sync All'), array('plugin' => 'spinomenal', 'controller' => 'spinomenal_admin', 'action' => 'syncGames'), array('class' => 'btn btn-primary')); ?>
                                        ...
                                        <div class="tab-content">
                                            <table class="table table-bordered table-striped">
                                                ...
                                                <?php foreach ($games as $row): ?>
                                                    <tr data-id="<?= $row['SpinomenalGames']['id']; ?>">
                                                        <td class="popper" data-popbox="pop1_<?= $row['SpinomenalGames']['id']; ?>"><?= $row['SpinomenalGames']['id']; ?></td>
                                                        <td><?= $row['SpinomenalGames']['game_id']; ?></td>
                                                        <td><input type="text" class="name-value" value="<?= $row['SpinomenalGames']['name'] ?>"/></td>
                                                        <td><input type="text" class="category-value" value="<?= $row['SpinomenalGames']['category'] ?>" /></td>
                                                        <td><?= $row['SpinomenalGames']['type']; ?></td>
                                                        <td><input type="text" class="paylines-value" value="<?= $row['SpinomenalGames']['paylines'] ?>"/></td>
                                                        <td><input type="text" class="reels-value" value="<?= $row['SpinomenalGames']['reels'] ?>" /></td>
                                                        <td><input type="text" class="image-value" value="<?= $row['SpinomenalGames']['image'] ?>" data-image="<?= $row['SpinomenalGames']['image'] ?>" /></td>
                                                        <!--<td><input type="text" class="branded-value" value="<?= $row['SpinomenalGames']['branded'] ?>" /></td>-->
                                                        <td><input class="checkbranded" type="checkbox" <?= ($row['SpinomenalGames']['branded'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfreespins" type="checkbox" <?= ($row['SpinomenalGames']['freespins'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkmobile" type="checkbox" <?= ($row['SpinomenalGames']['mobile'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkdesktop" type="checkbox" <?= ($row['SpinomenalGames']['desktop'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkfunplay" type="checkbox" <?= ($row['SpinomenalGames']['funplay'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checknew" type="checkbox" <?= ($row['SpinomenalGames']['new'] == 1) ? 'checked' : ""; ?> /></td>
                                                        <td><input class="checkactive" type="checkbox" <?= ($row['SpinomenalGames']['active'] == 1) ? 'checked' : ""; ?> /></td>
...
<script>
    $(document).ready(function () {
        $(document).on('click', '.adjust-game', function (e) {
            ...
            $.ajax({
                url: '/admin/spinomenal/spinomenal_admin/editGame/?id=' + _gid + '&name=' + _name + '&category=' + _category + '&paylines=' + _paylines +
