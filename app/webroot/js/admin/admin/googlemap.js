...
function initializemap(user) {
    ...
    var myLatlng = new google.maps.LatLng(25.363882,33.044922);
    ...
    jQuery.ajax({
        url : '/admin/Userlog/viewlog/'+userid,
        dataType : 'json',
      ...
      oms.addListener('click', function(marker) {
        ...
        infowindow.open(map, marker);
                ...
                for (p in places) {
                    ...
                    _generaldata = places[p].Userlog;
                    
                    _geodata = _generaldata.geolocdata;
                    
                    if (_geodata.latitude && _geodata.longitude){
                        //create gmap latlng obj
                        tmpLatLng = new google.maps.LatLng( _geodata.latitude, _geodata.longitude);
                        bounds.extend(tmpLatLng);
                        // make and place map maker.
                        var marker = new google.maps.Marker({
                        position: tmpLatLng,
                        map: map,
                        title:_generaldata.action + "@" + _generaldata.date + "\n\r" + _geodata.city +" - "+  _generaldata.ip
                        });

                       //bindInfoWindow(marker, map, infowindow, '<b>'+_generaldata.action + "@" + _generaldata.date + "</b><br>" + _geodata.city +  _generaldata.ip);
                        marker.desc = '<b>'+_generaldata.action + "@" + _generaldata.date + "</b><br>" + _geodata.city +" "+  _generaldata.ip;
